 ...
 * @param {Object} [options.pendingGuard] — локальные несинхронизированные значения; если отличаются от снапшота, поле не перезаписываем
 */
export function applyRemoteSnapshot(snapshot, ctx, options = {}) {
    if (!snapshot) return;
    ...
    const { pendingGuard } = options;

    const shouldSkipKey = (key, remoteVal) => {
        if (!pendingGuard || !(key in pendingGuard)) return false;
        try {
            return JSON.stringify(pendingGuard[key]) !== JSON.stringify(remoteVal);
        } catch (e) {
            return true;
        }
    };
    ...
    const applyField = (key, setter) => {
        ...
        if (shouldSkipKey(key, value)) return;
        setter((prev) => {
    ...
    if (Array.isArray(remotePlans)) {
        const skipPlans = shouldSkipKey(STORAGE_KEYS.SAVED_PLANS, remotePlans);
        if (!skipPlans) {
    ...
    if (serializedRegistry) {
        if (shouldSkipKey(STORAGE_KEYS.WORKER_REGISTRY, serializedRegistry)) return;
